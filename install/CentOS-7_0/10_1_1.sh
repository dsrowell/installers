...
echo -e "##############################################################"
echo -e "# Welcome to the unofficial ZPanelX Installer for CentOS 7.0 #"
echo -e "#                                                            #"
...
yum -y install ld-linux.so.2 libbz2.so.1 libdb-4.7.so libgd.so.2 httpd php php-suhosin php-devel php-gd php-mbstring php-mcrypt php-intl php-imap php-mysql php-xml php-xmlrpc curl curl-devel perl-libwww-perl libxml2 libxml2-devel mariadb-server zip webalizer gcc gcc-c++ httpd-devel at make mariadb-devel bzip2-devel postfix postfix-perl-scripts bash-completion dovecot dovecot-mysql dovecot-pigeonhole proftpd proftpd-mysql bind bind-utils bind-libs
...
if grep -q "Order [Aa]llow,[Dd]eny" /etc/zpanel/panel/modules/apache_admin/hooks/OnDaemonRun.hook.php; then sed -i 's/Order [Aa]llow,[Dd]eny/Require all granted/' /etc/zpanel/panel/modules/apache_admin/hooks/OnDaemonRun.hook.php; fi
if grep -q "Allow from all" /etc/zpanel/panel/modules/apache_admin/hooks/OnDaemonRun.hook.php; then sed -i '/Allow from all/d' /etc/zpanel/panel/modules/apache_admin/hooks/OnDaemonRun.hook.php; fi
